"""Foundational systems anchor labels from GENOME-ATLAS (genome-atlas v0.6.0).

These 16 systems are the gold-standard anchors for the training set:
all mechanism_bucket values were verified in GENOME-ATLAS (v1.2.0 audit).
Evidence weight: 1.0 (highest - directly verified in GENOME-ATLAS).
...
_NORMALISED = {
    k.replace(" ", "_").replace("(", "").replace(")", "").replace("__", "_"): v for k, v in SYSTEM_TO_TIER_B.items()
}
...
def main(output: Path = Path("/data/labels/evidence/foundational.parquet")) -> None:
    ...
    if len(is621):
        ...
        assert bool(row["composite_architecture"]) is True, "IS621 composite_architecture must be True!"
