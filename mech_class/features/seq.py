...
Two usage modes:
  1. Training / batch  - reuse the GENOME-ATLAS esm2_150M_v6.parquet (640-dim vectors).
...
def load_esm2_embeddings(path: Path = ESM2_PARQUET) -> pd.DataFrame:
    """Load GENOME-ATLAS precomputed ESM-2 embeddings parquet.
...
def build_seq_feature_matrix(
    ...
    *,
    ...
    """Build N x 640 ESM-2 feature matrix from accession list.
