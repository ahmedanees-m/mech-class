"""Holdout validation on 5 pre-registered probe proteins.
...
Feature computation strategy:
  seq_*   (640-dim): ESM-2 150M, computed fresh; Q99ZW2 from GENOME-ATLAS parquet.
...
def compute_features(probe: dict) -> np.ndarray:
    ...
    if cached is not None and len(cached) == ESM2_DIM:
        print(f"    ESM-2: using cached GENOME-ATLAS embedding for {acc}")
        seq_vec = cached
    else:
...
def run():
    print("=== Holdout Validation (corrected accessions) ===\n")
    ...
    print("  seq_*:    ESM-2 150M computed fresh (Q99ZW2 from GENOME-ATLAS parquet)")
