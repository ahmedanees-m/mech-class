...
Skip condition: trained model files not found at /data/models/tier_a/model.pkl.
  ...
  When trained models are present, all 10 probes are expected to pass.
...
pytestmark = pytest.mark.skipif(
    ...
    reason="Trained models not found - run after training",
)
...
_PROBES = [
    ...
    },
    {
        ...
        "composite": True,
        "canonical_pfam": ["PF01548", "PF02371"],
        # requires_esm2=False: Tier-A IS110 hard gate fires on PF01548 and PF02371 alone.
...
def test_iscro4_tier_a_gate_override(predictor):
    ...
    conf >= 0.90 whenever PF01548 and PF02371 are both present.
    """
...
def test_download_stub_raises_runtime_error(tmp_path):
    """_download_models must raise RuntimeError (stub; URL not yet configured)."""
    from mech_class.api import _download_models
